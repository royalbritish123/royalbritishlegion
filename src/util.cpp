...
#else
    const char* pszModule = "RoyalBritishLegion";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RoyalBritishLegion
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RoyalBritishLegion
    // Mac: ~/Library/Application Support/RoyalBritishLegion
    // Unix: ~/.RoyalBritishLegion
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RoyalBritishLegion";
#else
    fs::path pathRet;
    ...
    return pathRet / "RoyalBritishLegion";
#else
    // Unix
    return pathRet / ".RoyalBritishLegion";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "RoyalBritishLegion.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "RoyalBritishLegiond.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong RoyalBritishLegion will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("RoyalBritishLegion"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
