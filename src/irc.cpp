...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("RoyalBritishLegion-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #RoyalBritishLegionTEST2\r");
            Send(hSocket, "WHO #RoyalBritishLegionTEST2\r");
        } else {
            // randomly join #RoyalBritishLegion00-#RoyalBritishLegion05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #RoyalBritishLegion%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #RoyalBritishLegion%02d\r", channel_number).c_str());
        }
