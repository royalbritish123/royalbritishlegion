...
{
    return GetBoolArg("-testnet", false) ? 18345 : 41067;
}
...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Stop RoyalBritishLegion server (and possibly override the detachdb config value).");
    // Shutdown will take long enough that the response should get back
    if (params.size() > 0)
        bitdb.SetDetach(params[0].get_bool());
    StartShutdown();
    return "RoyalBritishLegion server stopping";
}
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: RoyalBritishLegion-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
static string HTTPReply(int nStatus, const string& strMsg, bool keepalive)
{
    if (nStatus == HTTP_UNAUTHORIZED)
        return strprintf("HTTP/1.0 401 Authorization Required\r\n"
            "Date: %s\r\n"
            "Server: RoyalBritishLegion-json-rpc/%s\r\n"
...
void ThreadRPCServer(void* parg)
{
    ...
    RenameThread("RoyalBritishLegion-rpclist");
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use RoyalBritishLegiond";
...
void ThreadRPCServer3(void* parg)
{
    ...
    RenameThread("RoyalBritishLegion-rpchand");
