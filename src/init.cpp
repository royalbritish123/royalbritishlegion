...
void Shutdown(void* parg)
{
    ...
    RenameThread("RoyalBritishLegion-shutoff");
...
//        CTxDB().Close();
        ...
        Sleep(50);
        printf("RoyalBritishLegion exited\n\n");
        fExit = true;
#ifndef QT_GUI
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("RoyalBritishLegion version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  RoyalBritishLegiond [options]                     " + "\n" +
                  "  RoyalBritishLegiond [options] <command> [params]  " + _("Send command to -server or RoyalBritishLegiond") + "\n" +
                  "  RoyalBritishLegiond [options] help                " + _("List commands") + "\n" +
                  "  RoyalBritishLegiond [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "RoyalBritishLegion:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("RoyalBritishLegion"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("RoyalBritishLegion"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: RoyalBritishLegion.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: RoyalBritishLegiond.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 41063 or testnet: 17778)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 41067 or testnet: 18345)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  RoyalBritishLegion is probably already running."), strDataDir.c_str()));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("RoyalBritishLegion version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "RoyalBritishLegion server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("RoyalBritishLegion"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of RoyalBritishLegion") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart RoyalBritishLegion to complete") << "\n";
